...
In this section we discuss the well posedness of the state equation for irregular sources from $L^2(I,H^{-1}(\Omega))$ which includes $\M$ according to the last section. For the proof of the well-posedness we need the following results for the linear KdV-equation.
\subsection{Well posedness of the linear Korteweg-de Vries}
First we consider
...
\eqref{constr y0} in the case $\gamma =0$. According to Remark \ref{ineqqnorm} the mapping $S$ is well-defined. In \cite{ClasonKaltenbacher13} the authors used a similar strategy for the definition of the control-to-state mapping since   The control-to-observation operator is denoted by
\[
