...
\]
endowed with the norm $\|\cdot\|_{\mathcal V}=\|\partial_{xx}\cdot\|_{L^2(\Omega)}$. %In particular it holds $H^2_0(\Omega)\subset\mathcal V$ and therefore $\mathcal V^*\subset H^{-2}(\Omega)$.
...
\end{remark}
\subsection{Well-posedness of the \KdVB equation}
...
Let $\gamma>0$. For any  $(f,y_0) \in \Hm1\times L^2(\Omega)$ let $y\in\mathcal B_\theta$, $\theta\in (0,T]$, be a time-local solution of \eqref{kdv1}-\eqref{kdv3}, then $y$ satisfies
 \be
 \norm{y}_{\mathcal B_t}\leq c\,\left( \frac{\sqrt{\gamma}+1}{\sqrt{\gamma}}\right) \left(\norm{y_{0}}_{L^{2}(\Omega)} + \frac{1}{\sqrt{\gamma}}\norm{f}_{L^2(I,H^{-1}(\Omega))}\right)
 \label{globalestimate}
 \ee
 for any $t\in(0,T]$ and some $c>0$ independent of $y$ and the data. Moreover there holds
 \[
 \|\partial_t y\|_{L^2((0,t),\mathcal V^*)}\leq C(T,L,y_0,f,\gamma).
 \]
\end{theorem}
\begin{proof}
We consider first that $y$ is a classical solution $y\in \mathcal C(\bar I,\mathcal D(A))\cap \mathcal\, C^1(\bar I,L^2(\Omega))$ for smooth data which exists according to \cite{faminskii2010initial}. Then equation \eqref{kdv1} holds in $L^2(\Omega)$ and we can multiply it with $y$ which yields
\[
...
\end{corollary}
\begin{proof}
First, we discuss uniqueness. Let $y_1$ and $y_2$ two solutions of \eqref{kdv1}-\eqref{kdv3}. We define $\delta y=y_1-y_2$. Then $\delta y$ solves equation \eqref{linkdv1}--\eqref{linkdv3} with $y=y_1-y_2$. Then Proposition \ref{prop:tangent} implies $y_1=y_2$.
\end{proof}
...
\ee
for $n$ large enough. Therefore $u_n$ is bounded, which implies the existence of an element $\bar u\in\M$ and a subsequence $u_{n_k}$ converging in the weak-$*$ topology of $\M$ towards $\bar u$. For each $u_{n_k}$, we define $y_{n_k}=Su_{n_k}\in \mathcal B\cap H^1(I,\mathcal V^*)$, cf., \cite[Corollary 3.30]{Brezis11}. Thanks to the global estimate \eqref{globalestimate}, there exists a exists a $\bar y\in L^2(I,H^1_0(\Omega))\cap\,L^\infty(I,L^2(\Omega))\cap\,H^1(I,\mathcal V^\ast)$ such that
