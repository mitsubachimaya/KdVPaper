...
\M & \quad \mbox{otherwise}
 \end{cases}$$
where $C$ is the constant from \eqref{ineqqnorm}. Moreover we introduce the control-to-state operator 
\[S\colon Q_{ad}\rightarrow \mathcal B\cap H^1(I,\mathcal V^\ast),\quad u\mapsto y\]
where $y$ is a weak solution of \eqref{kdvcontrol1}-\eqref{kdvcontrol3}. The control-to-observation operator is given by 
\[
S_{obs}\colon \M\rightarrow L^2(I\times \Omega_{obs})\times L^2(\Omega_{obs}),\quad u\mapsto(\chi_{\Omega_{obs}}y,\chi_{\Omega_{obs}}\partial_ty(T)) 
\] 
Thus we can rewrite problem \eqref{cost} in its reduced form given by 
\be
\min_{u \in \M}\frac{1}{2}\left(\norm{S_{obs}u - z}_{L^2(I\times \Omega_{obs})\times L^2(\Omega_{obs})}^2\right) + \alpha \norm{u}_{\M}.
\label{red cost}
\ee
\begin{prop}
