...
\begin{prop}
There exists a solution $(\bar y,\bar q) \in \mathcal B\times U_{ad}$ to the optimal control problem \eqref{cost}.
\end{prop}
\begin{proof}
\underline{Case $\gamma > 0$}. The cost function $J$ is a positive function. Thus its infimum $\bar J$ exists and there exists a minimizing sequence $(q_n,y_n) \in U_{ad} \times \mathcal B$ such that $J(q_n, y_n) \rightarrow \bar J$ as $n \rightarrow \infty$. Furthermore there exists a $\varepsilon>0$ such that
\be
