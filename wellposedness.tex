...
\label{diffpsi2}
\]
{\color{red} We choose a $\theta^\ast > 0$ such that
\begin{subequations}
 \begin{numcases}{}
 ...
 \end{numcases}
\end{subequations}
holds and consider $B = \{ y \in \mathcal{B}_{\theta^\ast}\colon\norm{y}_{\mathcal{B}_{\theta^\ast}} \leq r\}$. Therefore, we have 
\[
\norm{\Psi_{f,y_0}(y)}_{\mathcal{B}_{\theta^\ast}}\leq \frac 1 3 r+\frac 1{3r}r^2=\frac 2 3 r.
\]
and get
\[
...
Moreover, we have 
\[
\|\Psi_{f,y_0}(y) - \Psi_{f,y_0}(z)\|_{\mathcal{B}_{\theta^\ast}}\leq \frac 1{3r}2r\norm{y - z}_{\mathcal{B}_{\theta^\ast}}\leq \frac{2}{3}\norm{y - z}_{\mathcal{B}_{\theta^\ast}}.
\]
for all $(y,z) \in B$.}
...
since
\[
\int_\Omega y\partial_x y~\mathrm dx=0,\quad\int_\Omega y^2\partial_x y~\mathrm dx=0.
\]
...
\end{equation}
for some $C>0$ only depending on $L,T,y_0,f$ and $\gamma$.
\end{corollary}
\begin{proof}
First, we discuss uniqueness. Let $y_1$ and $y_2$ two solutions of \eqref{kdv1}-\eqref{kdv3}. We define $\delta y=y_1-y_2$. Then $\delta y$ solves equation \eqref{linkdv1}--\eqref{linkdv3} with $y=y_1+y_2$. Then \cref{prop:tangent} implies $y_1=y_2$. According to \cref{localposedness} equation \eqref{kdv1}--\eqref{kdv3} has a unique time-local solution $y\in \mathcal B_{T^\ast}$. Thus we can repeat the procedure of \cref{localposedness} starting from $y(T^\ast)$ until a maximal time of existence $0<t_{\max}\leq T$ is reached. If $t_{\max}<T$ it follows from \cite[Chapter 6, Theorem 2.2]{pazy1983semigroups} that $\lim_{t\uparrow t_{\max}}\|y(t)\|_{L^2(\Omega)}=\infty$ which contradicts \eqref{globalestimate}. The estimate \eqref{estytglobal} is proven using \eqref{globalestimate} and similar arguments as in the proof of \cref{existlocal}.
\qquad\end{proof}}

\section{Well-posedness of the optimization problem. Existence of an optimum}\label{sec:ex opt}
{\color{red} We start by defining the admissible set of controls
...
\underline{Case $\gamma = 0$}. In this case we rely on the estimate \eqref{localestimate} and use that the set $U_{ad}$ is weak-$*$ closed in $\M$.
\qquad\end{proof}
...
\begin{remark}\label{alphaconstraint}
 ...
 \[
\norm{u_n}_{\M} \leq \frac{c}{\alpha}\left(J(S(0),0) + \varepsilon \right).
 \]
