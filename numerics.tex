...
\ee
where the functions $\hat u_j(t)$ are pointwise evaluations in time of the control at the grid points and $\delta_{x_j}$ are Dirac functionals located at the grid points. Hence, we clearly have $u_N \in \M$ and can easily define any inner product required in the resolution of the state equation \eqref{discrcontrol}
\be
...
In the unregularized problem, the controls are discretized as nodal Dirac measures. In the regularized case, this is not possible anymore as we need a discretization that is compatible with $L^2(\Omega_c \times I)$ functions. Thus, for this regularized case, one need to find a discretization that should lead to an equivalent problem to \eqref{cost} in the limiting case $\gamma \to 0$ (i.e. to obtain a Dirac measure like discretization). We suggest to follow the strategy in \cite{pieper2014}. The control is discretized in space with piecewise linear, continuous finite elements on a grid whose nodes are the Chebyschev-Gauss-Lobatto points previsously mentioned.
...
&\langle u_h,\varphi_h \rangle = \sum_{n=0}^N{d_n \sum_{j=1}^{N_T} \Delta t \mathbf{\hat u}_{jn}\varphi_{jn}}
\label{masslumping}
\eeal
...
\caption{Spatial support of the control, determined by $\norm{p}_{L^2(0,T)}\geq \alpha$. Here $\alpha = 0.1$ and $\gamma = 10^{-6}$}
\label{support}
\end{figure}
\begin{figure}[!h]
...
\caption{Spatial support of the control, determined by $\norm{p}_{L^2(0,T)}\geq \alpha$, for $\gamma = 10^{-5}$.}
\label{supportIP}
\end{figure}
