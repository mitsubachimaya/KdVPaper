...
\be
\min_{u \in \mathcal{M}(\Omega_c,L^2(I)), y\in Y}J(y,u) =\frac{1}{2}\|\chi_{\Omega_{o}}y(T)-z_2\|_{L^2(\Omega_{o})}^2 +\alpha \norm{u}_{\mathcal{M}(\Omega_c, L^{2}(I))}
\ee
for different $\Omega_c$, $\Omega_o$ and $z_2$, where $y$ is the solution of \eqref{PhysicalKDV}. In particular, we emphasize that $U_{ad} = \mathcal{M}(\Omega_c,L^2(I))$.
