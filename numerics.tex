...
\be
J_{N,N_{T}}(\mathbf{y}_{0}, \ldots, \mathbf{y}_{N_{T}}, \mathbf{u}_{0}, \ldots, \mathbf{u}_{N_{T}}) = \frac{1}{2}\sum_{i=1}^{N_{T}}{\Delta t \mathbf{y}_{i}^{t}A \mathbf{y}_{i}} + \alpha \sum_{j=1}^N{\left( \sum_{i = 1}^{N_T}{\Delta t \mathbf{u}_{ij}^2}\right)^{1/2}}
\ee
...
\mathcal{L}_{N,N_{T}}(\mathbf{y}_{0}, \ldots, \mathbf{y}_{N_{T}}, \mathbf{u}_{0}, \ldots, \mathbf{u}_{N_{T}},\mathbf{p}_{0}, \ldots, \mathbf{p}_{N_{T}-1}) = J_{N,N_{T}}(\mathbf{y}_{0}, \ldots, \mathbf{y}_{N_{T}}, \mathbf{u}_{0}, \ldots, \mathbf{u}_{N_{T}}) + \\
 ...
 +\sum_{k=1}^{N-1}{\mathbf{p}_k^t\left[
 ...
 + \mathbf{p}_N^t\left[ M \mathbf{y}_{N+1} - \frac{1}{2}\left( M \mathbf{y}_N + M \mathbf{y}_{N-1} \right)- \frac{1}{2}\Delta t P \mathbf{y}_N - \frac{1}{2}\Delta t F(\mathbf{y}_N) +\frac{1}{2}\Delta t S \mathbf{y}_{N-1}\right].
 \end{multline}

and derive the discrete adjoint in the case of terminal observations
 \bealn
 &M^t \mathbf{p}_N = -A(y_{N+1} - y_d)\\
 &\frac{1}{2}\left( M^t+\Delta t S^t\right) \mathbf{p}_{N-1} = \frac{1}{2}\left( M^t + \Delta t P^t + \gamma \Delta t Q^t \right) \mathbf{p}_N + \frac{1}{2}\Delta t F^{'}(y_N)^t \mathbf{p}_N \\
 &\frac{1}{2}\left( M^t + \Delta t S^t\right)\mathbf{p}_{n-2} = \frac{1}{2}\left( M^t - \Delta t S^t\right)\mathbf{p}_{n} + \Delta t \left( P^t + \gamma Q^t\right)\mathbf{p}_{n-1} + \Delta t F^{'}(y_{n-1})^t \mathbf{p}_{n-1}, \\
 &\mbox{\hspace{0.7\textwidth}}\quad n=2 \ldots N.
\eealn
...
% The discrete adjoint scheme is obtained by imposing $\delta L(\bar y, \bar q, \bar p)\delta \bar y = 0$.
 ...
 &\mbox{\hspace{0.4\textwidth}}+ \Delta t F^{'}(y_{n-1})^t p_{n-1}, \quad n=2 \ldots N.
 \eealn
...
\label{normalmap}
\ee
for an auxiliary variable $q_{\gamma}$ such that $u_{\gamma} = P_{\gamma}(q_{\gamma})$. One easily prove in that case that $q_{\gamma} = -\frac{1}{\gamma}B^* p_{\gamma}$ and equivalence between \eqref{optcondition} and \eqref{normalmap}.
