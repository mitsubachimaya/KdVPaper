...
% In comparison to what is done in \cite{herzog2012directional}, the nonlinearity in our problem requires that we additionally compute the tangent and the second adjoint operators to get the gradient of $F$. Besides, in \cite{pieper2014}, the authors quickly prove that we obtain the original problem in the limiting case for great $\gamma$.
 To solve problem \eqref{normalmap}, we apply a Newton-type method, and since the proximal operator is not differentiable, we use the concept of semismoothness to derive a general derivative for $\operatorname{G}$ \cite{ulbrich2002semismooth}. To be more precise, we get
 \be
  \operatorname{DG}(q_{\rg}) = \rg \operatorname{Id} + \nabla^2 f( \operatorname{Prox}_{\psi/ \rg}(q_{\rg}))\operatorname{DProx}_{\psi/ \rg}(q_{\rg}),
 \ee
 where the general derivative of the proximal operator is given by
\begin{numcases}{\operatorname{DProx}_{\psi/ \rg}\delta q_{\rg} = }
\frac{1}{\rg}\left( \rg - \frac{\alpha}{\norm{q_{\rg}(x)}_{L^{2}(I)}}\right)^{+} \delta q_{\rg}(x) + \alpha \frac{\left( q_{\rg}, \delta q_{\rg}\right)_{L^2(I)}}{\norm{q_{\rg}(x)}_{L^{2}(I)}^3}q_{\rg} & \nonumber \\
