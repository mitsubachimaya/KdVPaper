...
\be
\operatorname{DG}(q_{\rg})\delta q = \rg\delta q + S_{obs}'^\star(y_\rg)\left(B(q_\rg)\cdot S_{obs}'(y_\rg)\operatorname{DProx}_{\psi/ \rg}(q_{\rg})\delta q\right),\quad \delta q\in L^2(I\times \Omega_c),
\ee
where $y_\rg=S_{obs}\operatorname{Prox}_{\psi/ \rg}(q_{\rg})$, $B(p_\rg)=1+\partial_xp_\rg(\operatorname{Prox}_{\psi/ \rg}(q_{\rg}))$, the operator $\cdot$ is the pointwise multiplication between two functions, and the generalized derivative of the proximal map is given by
