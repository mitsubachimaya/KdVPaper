...
\be
\bar u_{\gamma} = \frac{1}{\gamma}\shrink_{\alpha}(-\chi_{\Omega_c} \bar p_{\gamma}) = \frac{1}{\gamma}\left( (-\chi_{\Omega_c}\bar p_{\gamma} - \alpha)^{+} - (-\chi_{\Omega_c}\bar p_{\gamma} + \alpha)\right).
\ee
It is then directly possible to solve via a Newton type method the equation 
\be
F(u_{\gamma}) = u_{\gamma} - \frac{1}{\gamma}\shrink_{\alpha}(-\chi_{\Omega_c} \bar p_{\gamma}) = 0 .
\label{optcondition}
\ee
...
\label{PhysicalKDV}
\ee
where the parameter $f$ is proportional to $F-1$ ($F$ is the Froude number). Its value determines if a flow is subcritical ($f\leq 0$) or supercritical ($f> 0 $). The forcing $u$ can be interpreted as the derivative of the bottom topography. In the first example, an inverse problem, we want to find what bottom topography created the waves we observe at a final time T. We generate beforehand a wave on the time period $[0,T]$ with the following source term in a subcritical configuration ($f = -0.5$)

\begin{numcases}
{u(t,x) = }
 \nonumber 5\delta_{\{x = 1.5\}}, \mbox{ when } 0 < t\leq 2.5 \\
...
\begin{figure}[!h]
\includegraphics[width = 0.45\textwidth]{images/observationIP2.pdf}
\includegraphics[width = 0.45\textwidth]{images/observationIP.pdf}
...
The results are shown on Fig.\ref{supportcontrol1} and Fig.\ref{reconstruction}. The support of the control is, according to \eqref{}, included in the domains where $\|q\|_{} \geq \frac{\alpha}{\gamma}$. The continuation strategy enables to compare the differences with a high $L^2$ regularization term (low sparsity, Fig.\ref{supportcontrol1} - left) and a low $L^2$ regularization (hight sparsity, Fig.\ref{supportcontrol1} - right).
\begin{figure}[!h]
...
\begin{figure}[!h]
 \includegraphics[width = 0.8\textwidth]{images/gammma1res.png}
 \caption{State y, adjoint state p and control at the end of the optimization procedure for $\gamma = 1$}
\end{figure}

\begin{figure}[!h]
 \includegraphics[width = 0.8\textwidth]{images/gamma10000res.png}
 \caption{State y, adjoint state p and control at the end of the optimization procedure for $\gamma = 10^{-4}$}
\end{figure}

The second example is a control example. We divide the domain $\Omega = [-L,L]$ into two subdomains. The domain $\Omega_c = [-L,0]$ is the control domain, while $\Omega_{obs} = [0,L]$ is the observation domain. The flow enters the domain from the right. We want to create the wave profile shown on Fig.\ref{}
\begin{figure}[!h]
\includegraphics[width = 0.45\textwidth]{images/objectivewave.png}
