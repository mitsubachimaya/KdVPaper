...
% \end{rmk}
Next we introduce we adjoint control to state operator $S'^\star$. 
\begin{prop}
...
\begin{proof}
  ...
  reduced form
  \be \min_{q\in
    \M}\frac{1}{2}\left(\|S(q)-z_1\|_{L^2(I\times\Omega)}^2+\|S(q)(T)-z_2\|_{L^2(\Omega)}^2\right)+\alpha\|q\|_{\M}=G(q)+F(q).
  \label{reducedcost}
  \ee
