...
\begin{proof}
  We define 
  \[
  F(u_1,u_2)=\frac 1 2\left(\|u_1-z_1\|_{L^2(I\times \Omega_{obs})}^2+\|u_2-z_2\|_{L^2(\Omega_{obs})}^2\right)
  \]
  for $(u_1,u_2)\in L^2(I\times \Omega_o)\times L^2(I\times \Omega_o)$ and \[\psi(u)=\|u\|_{\M}+\mathcal I_{U_{ad}}(u).\]
  Since $F$ and $\SO$ are Fr\'echet differentiable the directional derivative of $F\circ \SO$ at $\bar u$ has the form
  \[
  D(F\circ \SO,\bar u,\delta =\langle S_{obs}'^\star(\bar u)(S_{obs}(\bar u)-z),\delta u\rangle_{\C,\M},\quad \delta u\in\C. 
  \]
  ...
  and in  particular
  \[
  F\circ \SO(\bar u)+\psi(\bar u)\leq G(\bar u + \varepsilon(u-\bar u))+\psi(\bar u+ \varepsilon(u-\bar u))
  \]
...
\begin{align*}
\phi^{\star}(p)&=\sup_{u\in U_{ad}}[\langle u,p\rangle_{\C,\M}-\alpha \|u\|_{\M}]\\
&=\sup_{\|u\|_{\M}=1,\,\lambda\in [0,\hat c]}\lambda[\langle u,p\rangle_{\C,\M}-\alpha].
\end{align*}
...
\be
\bar u'(x)=-\frac{\chi_{\Omega_c}\bar p(x)}{\alpha +\bar\lambda}\quad\text{in}~L^1((\Omega,|\bar u|),L^2(I)).
\label{controlintime}
\ee
...
\[
\int_{\Omega}(\bar u',\chi_{\Omega_c}\bar p)_{L^2(I)}-\|\bar u'\|_{L^2(I)}\|\chi_{\Omega_c}\bar p\|_{L^2(I)}~\mathrm d|\bar u|=0
\]
which implies \[\bar u'=-\frac 1{\|\chi_{\Omega_c}\bar p\|_{L^2(I)}}\,\chi_{\Omega_c}\bar p\quad\text{in}~ L^1((\Omega_c,|\bar u|),L^2(I)).\]
Moreover we have
\[
\int_\Omega\|\chi_{\Omega_c}\bar p\|_{L^2(I)}-\bar\lambda-\alpha~\mathrm d|\bar u|=0
\]
