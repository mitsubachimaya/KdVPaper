...
In this section we prove various estimates from the main part of the paper and establish the wellposedness of the linearized state equation as well as the adjoint linearized state equation.
...
\int_{0}^{L}{x(\partial_{x} y)^{2}~\mathrm dx}= \langle f,xy \rangle_{H^{1}_{0}(\Omega),H^{-1}(\Omega)}.
...
\]
{\color{red}
Next we choose $\theta^\ast$ small enough such that
\begin{equation}\label{linstatetheta}
\hat C\theta^{\ast 1/4}\|y\|_{\mathcal B}\leq\frac{1}{3}
\end{equation}
holds. Then the following inequalities hold
\[
\|\Psi_{\delta u, \delta y_0,y}(\delta y)\|_{\mathcal B_{\theta^\ast}}\leq \frac{2}{3}r,~~\|\Psi_{\delta u, \delta y_0,y}(\delta y_1) - \Psi_{\delta u, \delta y_0,y}(\delta y_2)\|_{\mathcal B_{\theta^\ast}}\leq\frac{1}{3}\|\delta y_1-\delta y_2\|_{\mathcal B_{\theta^\ast}}
\]
which imply that $\Psi_{\delta u, \delta y_0,y}$ is a contraction mapping on $B$. So we can apply the Banach fixed point theorem which guarantees the existence of a unique fixed point $\delta y$ of $\Psi_{\delta u, \delta y_0,y}$ which is a solution of \eqref{linkdv1}-\eqref{linkdv3} in $(0,\theta^\ast)$ with initial value $\delta y_0$. This strategy is repeated successively using the intermediate values of $y$ as initial data. The final time $T$ can be reached since the length $\theta^\ast$ of the existence intervals is independent of the initial data, see \eqref{linstatetheta}. The concatenation of all $\delta y$ for is a solution of \eqref{linkdv1} - \eqref{linkdv3}. This strategy generates a time grid $(t_k)_{k=0}^N$ with $t_0=0$, $t_N=T$ and $N=\lceil T/\theta^\ast\rceil$. Next we prove the estimate \eqref{estimatetangent}. Using the time-grid we get
\begin{align*}
...
\[
N\leq\frac{T}{\theta^\ast}+1=cT\|y\|_{\mathcal B}^4+1
\]
and accordingly
\[
