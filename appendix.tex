...
&\leq \widetilde{C}\left(\|\delta y_0\|_{L^2(\Omega)}+\|\delta u\|_{L^2(I,H^{-1}(\Omega))}\right) + \hat C\theta^{1/4}\|y\|_{\mathcal B_{\theta}}\|\delta y\|_{\mathcal B_{\theta}}
\end{align*}
and
\[
\|\Psi_{\delta u, \delta y_0,y}(\delta y_1)-\Psi_{\delta u, \delta y_0,y}(\delta y_2)\|_{\mathcal B_{\theta}}\leq \hat C\theta^{1/4}\|y\|_{\mathcal B_{\theta}}\|\delta y_1-\delta y_2\|_{\mathcal B_{\theta}}.
\]
...
\hat C\theta^{1/4}\|y\|_{\mathcal B_\theta}\leq\frac{1}{3}
\end{equation}
...
\]
which imply that $\Psi_{\delta u, \delta y_0,y}$ is a contraction mapping on $B$. So we can apply the Banach fixed point theorem which guarantees the existence of a unique fixed point $\delta y$ of $\Psi_{\delta u, \delta y_0,y}$ which is a solution of \eqref{linkdv1}-\eqref{linkdv3} in $(0,\theta)$ with initial value $\delta y_0$. Since $\theta$ is independent of $\delta y_0$ we can apply this strategy successively starting at $t=0$ with $\delta y_0$ and using $\delta y(k\theta)$ as initial points for $k=1,2,3,\ldots,N$ until $T$ is reached. The concatenation of all $\delta y(k\theta)$ for $k=1,2,3,\ldots,N$ is a solution of \eqref{linkdv1} - \eqref{linkdv3}. Next we discuss uniqueness. Let $\delta y\in \mathcal B$ be a solution of \eqref{linkdv1}--\eqref{linkdv3} for $\delta u=\delta y_0=0$. From the proof of \eqref{linestimate_regular} we see
\[
\|\delta y(t)\|_{L^2(\Omega)}\leq c\int_0^t\|\partial_x(\delta yy)(s)\|_{H^{-1}(\Omega)}~\mathrm ds\leq c\int_0^t\|y(s)\|_{H^1_0(\Omega)}\|\delta y(s)\|_{L^2(\Omega)}~\mathrm ds
\] 
for any $t>0$. Then Gronwall's Lemma implies $y\equiv0$. Thus existence of a unique solution is proven. The estimate \eqref{estimatetangent} follows from
\[
\|\delta y\|_{\mathcal B}\leq c\left(\sum_{k=1}^N\|\delta y\|_{\mathcal B_{k\theta}}+\|\delta y\|_{\mathcal B_{\theta^\ast}}\right)\leq c\,N\left(\|\delta u\|_{\Hm1}+\|\delta y\|_{L^2(\Omega)}\right),
\] 
where $N=\lfloor T/\theta\rfloor$ and $\theta^\ast=(T/\theta-N)\theta$. Then \eqref{linstatetheta} implies $N\leq \tilde c\|y\|_{\mathcal B}^4$ and accordingly
\[
\|\delta y\|_{\mathcal B}\leq c(\|y\|_{\mathcal B})\,\left(\|\delta u\|_{\Hm1}+\|\delta y\|_{L^2(\Omega)}\right).
\]   
...
\end{proposition}
\begin{proof}
Uniqueness follows from \cite[Proposition 15]{coron2003exact}. Existence and the estimate \eqref{apriori_adjoint} are proven in the same way as Proposition \ref{prop:tangent}. Hereby we rely on Lemma \ref{lemadjoint}.
